...
function RevokeTxDialog({open, handleClose, txIndex, transaction}:Props) {
   ...
   const getdata = async () => {
      try {
         ...
         !isApproved && setWarning("you should approve the transaction first!")
      } catch (error) {
   ...
   const handleRevoke = async() => {
      try{
         await contract?.methods.revoke(txIndex).send({from: account}) ;
         setRevoke(true)
         handleClose();
      } catch (e){
     ...
     <div>
       ...
       >
         ...
         <DialogActions>
           ...
           <Button onClick={handleRevoke} autoFocus disabled={!isOwner || !isApproved} size='small' variant='contained'>
             Revoke
           </Button>
         </DialogActions>
       </Dialog>
